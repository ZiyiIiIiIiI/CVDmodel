...
ui <- fluidPage(
  ...
  navbarPage(
    id = "cvd", theme = "custom-navbar.css",
    ...
    tabPanel(
      ...
      fluidRow(
        ...
        column(
          ...
          br(),
          p(tags$a(
            href = "https://heart.bmj.com/content/101/3/201", icon("mouse-pointer"),
            tags$strong("Lewsey JD, et al. A cardiovascular disease policy model that predicts life expectancy taking into account socioeconomic deprivation. Heart 2015;101:201-208. doi:10.1136/heartjnl-2014-305637.",
              style = "color:#2196c4; background-color: #FFFFFF"
            ), target = "_blank"
          )),
          p(tags$a(
            href = "https://openheart.bmj.com/content/3/1/e000140", icon("mouse-pointer"),
          ...
          p(
            tags$strong("Code for this model can be downloaded from Github repository by clicking"),
            tags$a(
              href = "https://github.com/yiqiaoxin/CVDmodel/",
              tags$strong("here", style = "color:#2196c4; background-color: #FFFFFF"), target = "_blank"
            ), tags$strong(".")
          ),
          br(),
          sidebarLayout(
            sidebarPanel(
              ...
              numericInput("simd",
                label = p(
                  "Scottish Index of Multiple Deprivation", tags$a(
                    href = "https://simd.scot/",
    ...
    tabPanel(
      ...
      column(
        ...
        br(),
        p(
          "Many previously developed models are often adapted to be used in multiple projects rather than
        ...
        br(),
        p(
          "Yiqiao Xin", tags$sup("1"), ", Ewan Gray", tags$sup("2"), ", Jose Antonio Robles-Zurita", tags$sup("1"), ", Houra Haghpanahan", tags$sup("1"), ", Robert Heggie", tags$sup("1"), ", Ciaran Kohli-Lynch", tags$sup("1"),
        ...
        br(),
        p(
          "We hope similar tools can be made for other health economics models in the future.
...
server <- function(input, output, session) {
  ...
  baseresult <- function() {
    ...
    list(
      base_nointvLY = base_nointv$LY, base_nointv_Cost = base_nointv$Cost, base_nointvQALY = base_nointv$QALY,
  ...
  output$cost <- renderValueBox({
    valueBox(
      format(baseresult()$base_nointv_Cost,
        big.mark = ",", big.interval = 3L,
        digits = 0, scientific = F
      ), "Health care costs (GBP)",
  ...
  output$intvqaly <- renderValueBox({
    valueBox(round(baseresult()$base_intv_allQALY - input$age, 2),
      paste(
        "Remaining quality-adjusted life-years, including",
  ...
  output$intvcost <- renderValueBox({
    valueBox(
      format(baseresult()$base_intv_allCost,
        big.mark = ",", big.interval = 3L,
        digits = 0, scientific = F
      ),
      paste(
        "Health care costs (GBP), including",
        format(baseresult()$statincost,
          big.mark = ",", big.interval = 3L,
          digits = 0, scientific = F
        ), "(GBP) statin cost"
      ),
  ...
  output$increcost <- renderValueBox({
    valueBox(
      format(baseresult()$incre_cost,
        big.mark = ",", big.interval = 3L,
        digits = 0, scientific = F
      ), "Incremental health care costs (GBP)",
